...
class PocketBaseService {
  ...
  Future<bool> isMovieDownloaded(String movieId) async {
    ...
    try {
      await pb.collection('movie_download').getFirstListItem(
        'id_user = "$userId" && link = "$movieId"',
      );
      return true;
    } catch (_) {
      return false;
  ...
  Future<void> addMovieToDownloads({
    ...
    // Daftar video sampel untuk tujuan demo.
    ...
    // Pilih video sampel secara dinamis berdasarkan ID film
  ...
  Future<void> deleteDownloadedMovie(String movieId) async {
    ...
    try {
      final record = await pb.collection('movie_download').getFirstListItem(
        'id_user = "$userId" && link = "$movieId"',
      );
  ...
  Future<List<Movie>> getMyDownloads() async {
    ...
    final records = await pb.collection('movie_download').getFullList(
